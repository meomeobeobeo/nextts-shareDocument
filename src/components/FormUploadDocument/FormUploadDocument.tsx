          ...
          <div className="flex flex-row justify-between items-center relative">

            {
              formView.map((format , index )=>{
                return <StatusForm formChange={handleSetFormBody} stepFomat={format} key={index} stepName="Upload" stepNumber={step} />
              })
            }

          </div>
          <FormBody formName = {formName} />
          <div onClick={handleIncreaseStep} className="p-2 bg-purple-400 rounded-lg cursor-pointer mt-1 absolute bottom-12 ">Next step</div>
        </div>
